...
async def escalate_to_p0(payload: dict):
    ...
    for vid, sess in simulator.slot_allocator.active_sessions.items():
        if vid == vehicle_id:
            if "ev" in sess:
                sess["ev"].user_priority = PriorityTier.P0_EMERGENCY
    ...
    for i, q_item in enumerate(simulator.slot_allocator.queue):
        if q_item[2].vehicle_id == vehicle_id:
            q_item[2].user_priority = PriorityTier.P0_EMERGENCY
            found_in_queue = True
            break
    ...
    if found_in_queue:
        ...
        for item in simulator.slot_allocator.queue:
            # Score is based on priority and other factors
            # In process_queue, it uses the score directly.
            # Here we just want to ensure it's at the front.
            priority_val = item[2].user_priority.value
            new_q.append((priority_val, item[1], item[2]))
